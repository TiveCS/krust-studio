...
- ~~Execute param input is text + explicit NULL toggle only~~ — done. `RoutineView`
  now picks a typed control per param (`classifyParam` → `ParamField`): enum/bool
  → `Combobox` (enum `creatable`), number → number input, date/timestamp/time →
...
- ~~No display-only **Pretty toggle** on the read-only definition viewer yet~~ —
  ...
  toggle, seeded from the global `prettySql` pref with a temporary non-persisted
  per-tab override (`prettyDefOverride`), mirroring the StructureView DDL toggle.
  Display-only — the stored/copied definition stays exact. The editable draft
  still uses its explicit **Format** action.
...
**~~Known sharp edge (accepted)~~ — fixed:** a PG `VOLATILE` (or MySQL `MODIFIES
SQL DATA`) function run via `SELECT` is still blocked on read-only connections.
`RoutineDef.volatile` is set in the pg (`provolatile = 'volatile'`) and mysql
(`SQL_DATA_ACCESS = 'MODIFIES SQL DATA'`) `getRoutine` mappings; `executeRoutine`
in `session.ts` fetches the def and throws for a volatile function on a read-only
connection (server-side guard), and the Execute button disables + explains it.
...
- [x] **Editable default value + auto-increment/identity.** Done. Per-column
      ...
      Auto-inc = create-time only. **Unsigned (MySQL/MariaDB) now done:**
      `NewColumnSpec.unsigned` emits `UNSIGNED` right after the type on CREATE
      (`buildCreateTable`) + `ADD COLUMN` (mysql driver); `ColumnsEditor` shows an
      **Unsigned** checkbox in the advanced panel for new columns when
      `engine === 'mysql'` (threaded from each host via the new `engine` prop).
...
- ~~New-table draft: leaving a typed-then-cleared PK cell as '' can fail on commit
  (sqlite)~~ — fixed. `NewTableEditor.create()` normalizes the draft before
  validating: trims names, drops whitespace-only defaults to `undefined` (never
  emits `DEFAULT ''`), and keeps a SQLite `pk` column with a blank type (it
  becomes `INTEGER PRIMARY KEY AUTOINCREMENT`) instead of filtering it out and
...
- ~~Auto-update "Restart now" didn't install~~ — fixed in 1.6.5 and verified by
  a successful real `1.6.5 → 1.6.6` update. The app downloaded, installed, and
  relaunched on 1.6.6 correctly. See ADR-0019.
