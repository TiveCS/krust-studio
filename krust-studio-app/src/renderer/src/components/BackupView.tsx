...
import type { BackupTableMode, BackupTableSpec, RestorePreview } from '../../../shared/types'
...
export function BackupView(): React.JSX.Element {
  ...
  useEffect(() => {
    const off = window.electron.ipcRenderer.on(
      'backup:progress',
      (_: unknown, p: { table: string; index: number; total: number }) => setProgress(p)
    )
    return () => off()
  }, [])
      ...
      <div className="min-h-0 flex-1 p-4">
        {panel === 'backup' ? (
          <div className="flex h-full flex-col gap-3">
            ...
            {/* table list — fills available space */}
          ...
          <div className="flex h-full flex-col gap-3">
            {readOnly && (
              <div className="flex shrink-0 items-center gap-2 rounded-md border border-amber-500/40 px-3 py-2 text-amber-500">
            ...
            <div className="flex shrink-0 items-center gap-3">
              <Button variant="outline" onClick={() => void chooseFile()} disabled={busy}>
                {busy && !preview ? <Loader2 className="animate-spin" /> : <Upload />}
                Choose a .sql backup…
              </Button>
              {restorePath && (
            ...
            {preview ? (
              <>
                <div className="flex shrink-0 items-center gap-3">
                  <span className="text-muted-foreground">{preview.total} statement(s)</span>
                ...
                {/* statement preview list — fills remaining height */}
                <div className="min-h-0 flex-1 overflow-auto rounded-md border border-border font-mono text-[11px]">
